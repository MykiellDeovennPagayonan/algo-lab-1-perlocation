...
export class Percolation {
  ...
  private quickUnion: QuickUnionUF;
  ...
  constructor(n: number, simulation: boolean) {
    ...
    this.quickUnion = new QuickUnionUF(n * n + 2);
    this.simulation = simulation;

    if (!this.simulation) {
      for (let i = 0; i < n; i++) {
        this.quickUnion.union(this.getIndex(0, i), n * n); //connects top row to the virtual top
        this.quickUnion.union(this.getIndex(n - 1, i), n * n + 1); //connects bottom row to the virtual bottom
      }
    }
  }
  ...
  open(row: number, col: number) {
    ...
    if (!this.isValid(row, col)) {
      throw new Error("Not valid row or column");
    }
    ...
    if (this.isValid(row - 1, col) && this.isOpen(row - 1, col)) {
      this.quickUnion.union(index, this.getIndex(row - 1, col));
    }
    ...
    if (this.isValid(row, col - 1) && this.isOpen(row, col - 1)) {
      this.quickUnion.union(index, this.getIndex(row, col - 1));
    }
    ...
    if (this.simulation) {
      if (row === 0) {
        this.quickUnion.union(this.getIndex(row, col), n * n); //connects top row to the virtual top
      }
      // scans the bottom rows if they are connected to the virtual top, if it is then it connects sa virtual bottom
      for (let i = 0; i < n; i++) {
        if (this.quickUnion.connected(this.getIndex(n - 1, i), n * n)) {
          this.quickUnion.union(this.getIndex(n - 1, i), n * n + 1);
        }
      }
    }
  }
  ...
  isFull(row: number, col: number): boolean {
    ...
    const n = this.size;
    return this.quickUnion.connected(this.getIndex(row, col), n * n);
  }
  ...
  percolates(): boolean {
    ...
    if (this.quickUnion.connected(n * n, n * n + 1)) {
      return true;
    }
    return false;
  }
